 ...
 * Modified for AppleWin
 *  TC did this
 * Modified for PolyUnit (August 2010)
 *  Andrew Trotman  (andrew@trotman.com)
 *
...
extern BYTE z80_memory_read(WORD addr);
extern void z80_memory_write(WORD addr, BYTE val);
extern BYTE z80_io_read(WORD addr);
extern void z80_io_write(WORD addr, BYTE val);

#define JUMP(addr)         (z80_reg_pc = (addr))
#define LOAD(addr)         z80_memory_read(addr)
#define STORE(addr, value) z80_memory_write(addr, value)
#define IN(addr)           z80_io_read(addr)
#define OUT(addr, value)   z80_io_write(addr, value)
...
static const double uZ80ClockMultiplier = CLK_Z80 / CLK_6809;
...
#else
	return (ULONG) ((double)uTStates / uZ80ClockMultiplier);
#endif
}
...
DWORD z80_step(void)
{
return z80_mainloop(1, maincpu_clk);
}
